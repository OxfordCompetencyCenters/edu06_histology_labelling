...
def generate_color_from_value(value):
    """
    Generates a visually distinct BGR color based on any value.
    """
...
def resolve_local_image_path(tile_path, images_dir):
    ...
    potential_paths = [
        ...
        os.path.join(images_dir, os.path.dirname(subpath), filename)
        ...
        for slide_dir in os.listdir(json_path):
            ...
            if os.path.isdir(slide_path):
                ...
                for pattern in patterns:
                    ...
                    if matches:
                        annotation_files_in_dir = [
                            f
                            for f in matches
                            if "annotation" in os.path.basename(f).lower()
                        ]
    ...
    for item in data:
        ...
        if filter_unclassified:
            ...
            cells = [
                ...
                for cell in cells
                if not (
                    (text_use_pred_class or color_by == "pred_class")
                    and cell.get("pred_class") == "Unclassified"
                )
            ]
            filtered_count = len(cells)
            if original_count != filtered_count:
                print(
                    f"Filtered {original_count - filtered_count} 'Unclassified' cells for {subpath}"
                )
        ...
        for cell in cells:
            ...
            if draw_bbox_flag and not bbox:
                print(
                    f"Warning: Skipping cell in {subpath} - Bbox drawing requested but bbox missing."
                )
                continue
            if draw_polygon_flag and not polygon:
                print(
                    f"Warning: Skipping cell in {subpath} - Polygon drawing requested but polygon missing."
                )
                continue
            ...
            if (
                not no_text and drawn_shape
            ):  # Only add text if enabled and a shape was drawn
                label_parts = []
                ...
                if label_parts:  # Only proceed if there's something to write
                    ...
                    (text_width, text_height), baseline = cv2.getTextSize(
                        label_str, cv2.FONT_HERSHEY_SIMPLEX, text_scale, 1
                    )
                    # Adjust background rectangle calculations
                    bg_y1 = text_pos[1] + baseline
                    bg_y2 = (
                        text_pos[1] - text_height - baseline // 2
        ...
        out_filename_safe = "".join(
            c if c.isalnum() or c in [".", "_", "-"] else "_" for c in out_filename
        )
        ...
        try:
            ...
            if success:
                print(
                    f"Annotated image saved to {output_path} ({label_count} labels drawn)"
                )
            else:
...
def main():
    ...
    parser.add_argument("--json_file", type=str, help="Path to the label JSON file.")
    parser.add_argument(
        "--json_dir", type=str, help="Directory containing the label JSON file."
    )
    parser.add_argument(
        "--images_dir",
        type=str,
    ...
    parser.add_argument(
        "--draw_bbox", action="store_true", help="If set, draw bounding boxes."
    )
    parser.add_argument(
        "--draw_polygon",
        action="store_true",
    ...
    if not (args.draw_bbox or args.draw_polygon):
        print(
            "Warning: Neither --draw_bbox nor --draw_polygon specified. No shapes will be drawn."
        )
    ...
    text_flags_specified = (
        args.text_use_pred_class
        or args.text_use_cluster_id
        or args.text_use_cluster_confidence
    )

    if (
        not args.no_text
        and not text_flags_specified
        and (args.draw_bbox or args.draw_polygon)
    ):
        print(
            "Info: No text components (--text_use_*) selected. Only shapes will be drawn."
        )

    if args.no_text and text_flags_specified:
        print(
            "Warning: --no_text is set but text flags (--text_use_*) are also specified. Text flags will be ignored."
        )
    elif text_flags_specified and not args.no_text:
        print(
            f"Info: Text will be displayed with the following components: "
