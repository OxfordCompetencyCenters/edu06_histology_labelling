...
def analyze_cluster_and_tiles_per_slide(
    ...
    if cluster_analyzer_max_items is not None:
        for cluster_id in clusters_dict:
            clusters_dict[cluster_id] = clusters_dict[cluster_id][
        ...
        for cell in tile.get("cells", []):
            if cell.get("cell_id") in filtered_cell_ids:
                if tile_path not in tile_map:
                    new_tile = {
                        key: value for key, value in tile.items() if key != "cells"
                    }
                    new_tile["cells"] = []
    ...
    print(
        f"Filtered annotations for slide {slide_name} saved to: {filtered_annotations_path}"
    )
    ...
    print(
        f"Cluster analysis results for slide {slide_name} saved to: {cluster_analysis_path}"
    )
...
def main():
    ...
    for slide_name, json_path in annotation_files.items():
        ...
        print(
            f"Completed processing slide {slide_name}: {len(filtered_data)} tiles with filtered cells"
        )
