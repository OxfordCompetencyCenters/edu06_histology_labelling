...
from utils import (
    build_slide_to_bbox_files_mapping,
    log_and_print,
    log_exception,
    mark_slide_done,
    is_slide_done,
...
def init():
    ...
    parser.add_argument(
        "--gpu", action="store_true", help="Use GPU for embedding extraction and DBSCAN"
    )
    ...
    parser.add_argument(
        "--umap_n_components", type=int, default=50, help="Target dimensions for UMAP"
    )
    parser.add_argument(
        "--umap_n_neighbors", type=int, default=15, help="UMAP n_neighbors parameter"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--auto_eps_sensitivity",
        type=float,
        default=1.0,
        help="Sensitivity for kneed elbow detection",
    )
    parser.add_argument(
        "--auto_eps_fallback_percentile",
        type=float,
        default=95.0,
    ...
    if _args.gpu and torch.cuda.is_available():
        ...
        if _args.gpu:
            log_and_print(
                "GPU requested but CUDA not available. Using CPU.", level="warning"
            )
...
def extract_patch_embedding(
    tile_img: Image.Image, bbox: List[int]
) -> Optional[np.ndarray]:
...
def find_optimal_eps(
    embeddings: np.ndarray, min_samples: int, output_path: str
) -> Optional[float]:
    ...
    try:
        nbrs = NearestNeighbors(
            n_neighbors=effective_k, algorithm="auto", metric="euclidean"
        ).fit(embeddings)
        ...
        fallback_eps = np.percentile(
            sorted_kth_distances, _args.auto_eps_fallback_percentile
        )
        if fallback_eps < 1e-6:
            fallback_eps = (
                np.mean(sorted_kth_distances[sorted_kth_distances > 1e-6])
                if np.any(sorted_kth_distances > 1e-6)
                else 0.1
            )
...
def cluster_slide(slide_id: str) -> dict:
    ...
    if not bbox_tile_pairs:
        log_and_print(
            f"No valid bbox/tile pairs found for slide {slide_id}", level="warning"
        )
        return {
            "slide_id": slide_id,
            "num_embeddings": 0,
    ...
    for bbox_file, tile_path in bbox_tile_pairs:
        try:
            ...
            for bb_dict in bboxes_data:
                if (
                    not isinstance(bb_dict, dict)
                    or "label_id" not in bb_dict
                    or "bbox" not in bb_dict
                ):
                    continue
    ...
    if _args.use_umap:
        try:
            ...
            log_and_print(
                f"UMAP reduced to {embeddings_np.shape[1]} dimensions for {slide_id}"
            )
        except Exception as e:
    ...
    if len(embeddings_np) < _args.min_samples:
        log_and_print(
            f"Too few embeddings ({len(embeddings_np)}) for DBSCAN on {slide_id}",
            level="warning",
        )
        return {
            "slide_id": slide_id,
    ...
    if chosen_eps is None:
        chosen_eps = find_optimal_eps(
            embeddings_np, _args.min_samples, str(slide_output_path)
        )
        if chosen_eps is None or chosen_eps <= 0:
            chosen_eps = 0.5  # Default fallback
            log_and_print(
                f"Using fallback eps={chosen_eps} for {slide_id}", level="warning"
            )
    ...
    if not use_gpu_dbscan:
        dbscan_cpu = CPUDbscan(
            eps=chosen_eps,
            min_samples=_args.min_samples,
            metric=_args.dbscan_metric,
            n_jobs=-1,
        )
    ...
    log_and_print(
        f"Clustering results for {slide_id}: {n_clusters} clusters, {n_noise} noise points"
    )
...
def run(mini_batch: List[str]) -> List[str]:
    ...
    for trigger_file in mini_batch:
        ...
        if slide_id not in _slide_to_files:
            log_and_print(
                f"Slide ID '{slide_id}' not in pre-built mapping, skipping",
                level="warning",
            )
        ...
        try:
            ...
            log_and_print(
                f"Finished clustering {slide_id}: {result_info['num_clusters']} clusters from {result_info['num_embeddings']} embeddings"
            )
...
def run_all() -> None:
    ...
    if _slide_filter:
        ...
        slide_ids = [
            sid for sid in slide_ids if should_process_slide(sid, _slide_filter)
        ]
        log_and_print(
            f"[Sequential] Slide filter matched {len(slide_ids)}/{before} slides"
        )
    ...
    log_and_print(
        f"[Sequential] Clustering complete. {len(all_results)} slides processed."
    )
    shutdown()
