...
def load_cluster_assignments_for_slide(slide_name: str) -> Optional[List[Dict]]:
    ...
    slide_cluster_path = os.path.join(
        _args.clustered_cells_path, slide_name, "cluster_assignments.json"
    )
    if os.path.exists(slide_cluster_path):
        try:
            ...
            log_exception(
                f"Failed to load cluster assignments from {slide_cluster_path}", e
            )
...
def classify_slide(slide_id: str) -> dict:
    ...
    if clustered_cells:
        log_and_print(
            f"Using cluster assignments for slide {slide_id}: {len(clustered_cells)} cells"
        )
        ...
        for cluster_id, entries in cluster_map.items():
            sorted_entries = sorted(
                entries, key=lambda e: e.get("confidence", 0.0), reverse=True
            )
            top_n = sorted_entries[: _args.classify_per_cluster]
            selected_for_classification.extend(top_n)

        log_and_print(
            f"Selected {len(selected_for_classification)} cells for multimodal-LLM classification"
        )
        ...
        for tile_path, entries_in_tile in bboxes_by_tile.items():
            if not os.path.exists(tile_path):
                ...
                for entry in entries_in_tile:
                    multimodal_llm_predictions[
                        (entry["bbox_file"], entry["label_id"])
            ...
            try:
                ...
                log_exception(
                    f"Error processing tile {tile_path} (classification batch)", e
                )
                for entry in entries_in_tile:
                    cell_id = (entry["bbox_file"], entry["label_id"])
                    if cell_id not in multimodal_llm_predictions:
                        multimodal_llm_predictions[cell_id] = (
        ...
        log_and_print(
            f"No cluster assignments found for {slide_id}. Classifying all bboxes."
        )
        ...
        for bbox_file in bbox_files:
            ...
            try:
                ...
                log_exception(
                    f"Error processing tile {tile_path} (no-cluster classification)", e
                )
    ...
    log_and_print(
        f"Classification for {slide_id}: {len(all_cell_results)} cells total, {num_classified} classified by multimodal-LLM"
    )
...
def run_all() -> None:
    ...
    log_and_print(
        f"[Sequential] Classification complete. {len(all_results)} slides processed."
    )
    shutdown()
