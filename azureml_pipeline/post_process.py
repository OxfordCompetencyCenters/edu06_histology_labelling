...
def parse_tile_filename(filename: str) -> dict | None:
    ...
    if (
        "__MAG_" not in basename
        or "__X_" not in basename
        or "__Y_" not in basename
        or "__IDX_" not in basename
    ):
        return None
...
def main():
    ...
    if slide_filter:
        ...
        logging.info(
            "Slide filter matched %d/%d slides", len(classification_files), before
        )

    logging.info(
        "Found classification files for slides: %s", list(classification_files.keys())
    )
    ...
    for slide_name, cls_file in classification_files.items():
        ...
        for tile in classification_results:
            ...
            if meta is None:
                logging.warning(
                    "Could not parse magnification from %s – skipping.", tile_name
                )
                continue
            ...
            for poly in polys:
                ...
                cls_cell = next(
                    (c for c in tile["classified_cells"] if c["label_id"] == lbl_id),
                    None,
                )
                if cls_cell is None:
        ...
        out_name = (
            f"v1_{args.param_string}_annotations.json"
            if args.param_string
            else "final_annotations.json"
        )
