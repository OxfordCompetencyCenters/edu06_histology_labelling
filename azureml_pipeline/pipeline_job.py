...
def build_param_string(args):
    ...
    if start_from in ["cluster_tiles"]:
        if hasattr(args, "enable_cluster_tiles") and args.enable_cluster_tiles:
            ...
            parts.append(
                f"reptiles_{format_param_for_name(args.cluster_analyzer_confidence_threshold)}"
            )
            if args.cluster_analyzer_max_items is not None:
                parts.append(f"maxper_{args.cluster_analyzer_max_items}")
        if (
            hasattr(args, "enable_filtered_annotations")
            and args.enable_filtered_annotations
        ):
...
def build_components(
    *,
    ...
    logging.info(
        f"Parallelization settings: max_nodes={max_nodes}, processes_per_node={processes_per_node}"
    )
    if slide_filter:
    ...
    data_prep_component = parallel_run_function(
        ...
        inputs=dict(
            input_data=Input(
                type=AssetTypes.URI_FOLDER, description="Input folder with WSI files"
            ),
        ),
        outputs=dict(
            output_path=Output(type=AssetTypes.URI_FOLDER, path=data_prep_output_uri),
            manifest_path=Output(
                type=AssetTypes.URI_FOLDER, path=data_prep_manifest_uri
            ),
        ),
        ...
        tile_filter_component = parallel_run_function(
            ...
            inputs=dict(
                trigger_path=Input(
                    ...
                    description="Manifest triggers",
                    mode="ro_mount",
                ),
                input_path=Input(
                    type=AssetTypes.URI_FOLDER,
                    description="Input tiles",
                    mode="ro_mount",
                ),
            ),
            outputs=dict(
                output_path=Output(
                    type=AssetTypes.URI_FOLDER, path=tile_filter_output_uri
                ),
                output_manifest=Output(
                    type=AssetTypes.URI_FOLDER, path=tile_filter_manifest_uri
                ),
            ),
    ...
    segment_component = parallel_run_function(
        ...
        inputs=dict(
            ...
            prepped_tiles_path=Input(
                type=AssetTypes.URI_FOLDER, description="Input tiles", mode="ro_mount"
            ),
        ),
        outputs=dict(
            output_path=Output(type=AssetTypes.URI_FOLDER, path=segment_output_uri),
            output_manifest=Output(
                type=AssetTypes.URI_FOLDER, description="Manifest for downstream steps"
            ),
        ),
        input_data="${{inputs.trigger_path}}",
        instance_count=max_nodes,
    ...
    cluster_component = parallel_run_function(
        ...
        inputs=dict(
            ...
            segmentation_path=Input(
                type=AssetTypes.URI_FOLDER,
                description="Segmentation results folder",
                mode="ro_mount",
            ),
            prepped_tiles_path=Input(
                type=AssetTypes.URI_FOLDER,
                description="Prepped tiles folder",
                mode="ro_mount",
            ),
        ),
        outputs=dict(
        ...
        task=RunFunction(
            ...
            program_arguments=(
                ...
                + (
                    ...
                    f"--umap_metric {cluster_umap_metric} "
                    if cluster_use_umap
                    else ""
                )
            ),
    ...
    classify_component = parallel_run_function(
        ...
        inputs=dict(
            ...
            cluster_path=Input(
                type=AssetTypes.URI_FOLDER,
                description="Clustering results folder",
                mode="ro_mount",
            ),
        ),
        outputs=dict(
    ...
    post_process_component = command(
        ...
        outputs={
            "output_path": Output(
                type=AssetTypes.URI_FOLDER, path=postprocess_output_uri
            )
        },
        code="./",
        command=post_cmd,
    ...
    if enable_annotations:
        ...
        use_text = (
            annotation_text_use_pred_class
            or annotation_text_use_cluster_id
            or annotation_text_use_cluster_confidence
        )
        annotation_cmd = (
        ...
        annotation_component = command(
            ...
            outputs={
                "output_path": Output(
                    type=AssetTypes.URI_FOLDER, path=annotation_output_uri
                )
            },
            code="./",
    ...
    if enable_cluster_tiles:
        ...
        cluster_tiles_component = command(
            ...
            outputs={
                "output_path": Output(
                    type=AssetTypes.URI_FOLDER, path=cluster_tiles_output_uri
                )
            },
            code="./",
    ...
    if enable_filtered_annotations and enable_cluster_tiles:
        # Don't use --no_text if we want to show cluster IDs or other text
        filtered_use_text = (
            filtered_annotation_text_use_pred_class
            or filtered_annotation_text_use_cluster_id
            or filtered_annotation_text_use_cluster_confidence
        )
        ...
        filtered_annotation_component = command(
            ...
            outputs={
                "output_path": Output(
                    type=AssetTypes.URI_FOLDER, path=filtered_annotation_output_uri
                )
            },
            code="./",
...
def build_sequential_components(
    *,
    ...
    data_prep_component = command(
        ...
        outputs={
            "output_path": Output(
                type=AssetTypes.URI_FOLDER, path=data_prep_output_uri, mode="rw_mount"
            ),
            "manifest_path": Output(
                type=AssetTypes.URI_FOLDER, path=data_prep_manifest_uri, mode="rw_mount"
            ),
        },
        code="./",
    ...
    if filter_tiles:
        ...
        tile_filter_component = command(
            ...
            outputs={
                "output_path": Output(
                    type=AssetTypes.URI_FOLDER,
                    path=tile_filter_output_uri,
                    mode="rw_mount",
                ),
                "output_manifest": Output(
                    type=AssetTypes.URI_FOLDER,
                    path=tile_filter_manifest_uri,
                    mode="rw_mount",
                ),
            },
            code="./",
    ...
    segment_component = command(
        ...
        outputs={
            "output_path": Output(
                type=AssetTypes.URI_FOLDER, path=segment_output_uri, mode="rw_mount"
            ),
    ...
    cluster_component = command(
        ...
        outputs={
            "cluster_output": Output(
                type=AssetTypes.URI_FOLDER, path=cluster_output_uri, mode="rw_mount"
            ),
            "output_manifest": Output(type=AssetTypes.URI_FOLDER, mode="rw_mount"),
        },
        code="./",
    ...
    classify_component = command(
        ...
        outputs={
            "output_path": Output(
                type=AssetTypes.URI_FOLDER, path=classify_output_uri, mode="rw_mount"
            ),
        },
        code="./",
    ...
    post_process_component = command(
        ...
        outputs={
            "output_path": Output(
                type=AssetTypes.URI_FOLDER, path=postprocess_output_uri, mode="rw_mount"
            )
        },
        code="./",
        command=post_cmd,
    ...
    if enable_filtered_annotations and enable_cluster_tiles:
        ...
        filtered_annotation_component = command(
            ...
            outputs={
                "output_path": Output(
                    type=AssetTypes.URI_FOLDER,
                    path=filtered_annotation_output_uri,
                    mode="rw_mount",
                )
            },
            code="./",
...
def run_pipeline():
    ...
    p.add_argument(
        "--raw_slides_uri",
        default="azureml://datastores/workspaceblobstore/paths/your_slides/",
    )
    p.add_argument(
        "--prepped_data_uri",
        default=None,
    ...
    p.add_argument(
        "--annotation_max_labels",
        type=int,
        default=100,
        help="Maximum number of labels to draw per image",
    )
    p.add_argument(
        "--annotation_random_labels",
        action="store_true",
        help="Pick labels randomly up to max_labels",
    )
    p.add_argument(
        "--annotation_draw_bbox", action="store_true", help="Draw bounding boxes"
    )
    p.add_argument(
        "--annotation_draw_polygon",
        action="store_true",
        default=True,
        help="Draw polygons",
    )
    ...
    p.add_argument(
        "--annotation_text_use_cluster_id",
        action="store_true",
        help="Include cluster ID in text labels",
    )
    p.add_argument(
        "--annotation_text_use_cluster_confidence",
        action="store_true",
    ...
    p.add_argument(
        "--annotation_filter_unclassified",
        action="store_true",
        default=True,
    ...
    p.add_argument(
        "--cluster_analyzer_confidence_threshold",
        type=float,
        default=0.75,
        help="Minimum cluster confidence for tile extraction [0.75]",
    )
    p.add_argument(
        "--cluster_analyzer_max_items",
        type=int,
        default=20,
    ...
    p.add_argument(
        "--use_separate_clustering_cluster",
        action="store_true",
    ...
    cluster_tiles_uri = (
        f"{base_uri}/cluster_tiles/" if args.enable_cluster_tiles else None
    )
    filtered_annotation_uri = (
        f"{base_uri}/filtered_annotations/"
        if args.enable_filtered_annotations
        else None
    )
    ...
    if use_sequential:
        logging.info(
            "max_nodes=1 → using SEQUENTIAL (command-based) pipeline for easier debugging"
        )

    component_builder = (
        build_sequential_components if use_sequential else build_components
    )
    components = component_builder(
        ...
        clustering_use_gpu=args.clustering_use_gpu
        if args.clustering_use_gpu
        else args.cluster_use_gpu,
    ...
    if args.use_separate_clustering_cluster:
        ...
        if not args.clustering_use_gpu and "cpu" in COMPUTE_CLUSTER_CLUSTERING.lower():
            ...
            logging.info(
                f"Auto-detected CPU-only clustering cluster: {COMPUTE_CLUSTER_CLUSTERING}"
            )
    ...
    def pipe_data_prep_seq(raw_slides_input):
        ...
        clu = components["cluster"](
            segmentation_path=seg.outputs.output_path, prepped_tiles_path=tiles
        )
        if args.use_separate_clustering_cluster:
            clu.compute = clustering_cluster_target
        cls = components["classify"](
    ...
    # ---- segment ----
    @pipeline(
        compute=COMPUTE_CLUSTER, description="From segmentation onward (parallel)"
    )
    def pipe_segment(prepped_in, manifest_in):
        seg = components["segment"](
            trigger_path=manifest_in, prepped_tiles_path=prepped_in
        )
        clu = components["cluster"](
            trigger_path=seg.outputs.output_manifest,
            segmentation_path=seg.outputs.output_path,
    ...
    @pipeline(
        compute=COMPUTE_CLUSTER, description="From segmentation onward (sequential)"
    )
    def pipe_segment_seq(prepped_in):
        seg = components["segment"](prepped_tiles_path=prepped_in)
        clu = components["cluster"](
            segmentation_path=seg.outputs.output_path, prepped_tiles_path=prepped_in
        )
        if args.use_separate_clustering_cluster:
            clu.compute = clustering_cluster_target
        cls = components["classify"](
    ...
    def pipe_cluster(prepped_in, segmented_in, seg_manifest_in):
        clu = components["cluster"](
            trigger_path=seg_manifest_in,
            segmentation_path=segmented_in,
            prepped_tiles_path=prepped_in,
        )
        if args.use_separate_clustering_cluster:
            clu.compute = clustering_cluster_target
        cls = components["classify"](
            trigger_path=clu.outputs.output_manifest,
            segmented_path=segmented_in,
            prepped_tiles_path=prepped_in,
            cluster_path=clu.outputs.cluster_output,
        )
        post = components["post_process"](
            segmentation_path=segmented_in, classification_path=cls.outputs.output_path
        )
        outputs = {"final_output": post.outputs.output_path}
        _append_tail_steps(outputs, post.outputs.output_path, prepped_in)
        return outputs
    ...
    @pipeline(
        compute=COMPUTE_CLUSTER, description="From clustering onward (sequential)"
    )
    def pipe_cluster_seq(prepped_in, segmented_in):
        clu = components["cluster"](
            segmentation_path=segmented_in, prepped_tiles_path=prepped_in
        )
        if args.use_separate_clustering_cluster:
            clu.compute = clustering_cluster_target
        cls = components["classify"](
    ...
    # ---- classify ----
    @pipeline(
        compute=COMPUTE_CLUSTER, description="From classification onward (parallel)"
    )
    def pipe_classify(prepped_in, segmented_in, clustered_in, clu_manifest_in):
        cls = components["classify"](
            trigger_path=clu_manifest_in,
            segmented_path=segmented_in,
            prepped_tiles_path=prepped_in,
            cluster_path=clustered_in,
        )
        post = components["post_process"](
            segmentation_path=segmented_in, classification_path=cls.outputs.output_path
        )
    ...
    @pipeline(
        compute=COMPUTE_CLUSTER, description="From classification onward (sequential)"
    )
    ...
    def pipe_post_process(prepped_in, segmented_in, classified_in):
        post = components["post_process"](
            segmentation_path=segmented_in, classification_path=classified_in
        )
        outputs = {"final_output": post.outputs.output_path}
        _append_tail_steps(outputs, post.outputs.output_path, prepped_in)
        return outputs
    ...
    # ---- annotate ----
    @pipeline(
        compute=COMPUTE_CLUSTER, description="Annotate existing post-process results"
    )
    def pipe_annotate(postprocess_in, prepped_in):
        outputs = {}
        if "annotation" in components:
            annotate = components["annotation"](
                annotations_json=postprocess_in, prepped_tiles_path=prepped_in
            )
            outputs["annotations"] = annotate.outputs.output_path
        return outputs
    ...
    @pipeline(
        ...
        description="Extract cluster tiles (and filtered annotations)",
    )
    ...
    if step == "data_prep":
        ...
        logging.info(
            "tile_filter start is equivalent to running --start_from segment with --filter_tiles"
        )
        if use_sequential:
            ...
            if not args.prepped_manifest_uri:
                logging.error(
                    "Parallel start_from=tile_filter requires --prepped_manifest_uri"
                )
                return
            job = pipe_segment(
                prepped_in=_inp(args.prepped_data_uri),
                manifest_in=_inp(args.prepped_manifest_uri),
            )
        ...
        if use_sequential:
            ...
            if not args.prepped_manifest_uri:
                logging.error(
                    "Parallel start_from=segment requires --prepped_manifest_uri"
                )
                return
            job = pipe_segment(
                prepped_in=_inp(args.prepped_data_uri),
        ...
        if use_sequential:
            ...
            if not args.segmented_manifest_uri:
                logging.error(
                    "Parallel start_from=cluster requires --segmented_manifest_uri"
                )
                return
            job = pipe_cluster(
                prepped_in=_inp(args.prepped_data_uri),
        ...
        if use_sequential:
            ...
            if not args.clustered_manifest_uri:
                logging.error(
                    "Parallel start_from=classify requires --clustered_manifest_uri"
                )
                return
            job = pipe_classify(
                prepped_in=_inp(args.prepped_data_uri),
        ...
        if not args.enable_cluster_tiles:
            logging.error(
                "start_from=cluster_tiles requires --enable_cluster_tiles flag"
            )
            return
...
def validate_environment(args: argparse.Namespace):
    ...
    if step in (
        ...
        if not args.prepped_data_uri:
            uri_errors.append(
                "--prepped_data_uri is required when starting from '{}'".format(step)
            )
    if step in ("cluster", "classify", "post_process"):
        if not args.segmented_data_uri:
            uri_errors.append(
                "--segmented_data_uri is required when starting from '{}'".format(step)
            )
    if step == "classify":
        if not args.clustered_data_uri:
            uri_errors.append(
                "--clustered_data_uri is required when starting from '{}'".format(step)
            )
    if step == "post_process":
        if not args.classified_data_uri:
            uri_errors.append(
                "--classified_data_uri is required when starting from '{}'".format(step)
            )
    if step in ("annotate", "cluster_tiles"):
        if not args.postprocess_data_uri:
            uri_errors.append(
                "--postprocess_data_uri is required when starting from '{}'".format(
                    step
                )
            )
