...
def tile_slide_multi_resolution(slide_path, out_dir, tile_sizes=[256, 512], overlap=0, target_mpp=0.5):
    """
    Enhanced tiling function that creates tiles at multiple resolutions.
...
def tile_slide(slide_path, out_dir, tile_size=512, overlap=0):
    """
    Legacy function for backward compatibility.
    Calls the enhanced multi-resolution tiling with single tile size.
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced data preparation for histology analysis")
    ...
    logging.info("Starting enhanced data prep with arguments: %s", args)
    ...
    logging.info("Enhanced data prep step done. Tiles saved to: %s", args.output_path)
